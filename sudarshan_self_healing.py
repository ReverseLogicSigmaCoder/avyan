...
import os
def sanitize_healing_path(p):
    return os.path.abspath(os.path.join(".", os.path.basename(p)))
import ast
import os
import sys
import traceback
...
"""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10, timeout=10) as response:
...
def apply_patch_and_create_pr(file_path, original_code, faulty_line, line_num, fix_code):
    ...
    with open(sanitize_healing_path(backup_path), "w", encoding="utf-8") as f:
    ...
    if 0 <= line_num - 1 < len(lines):
        ...
        with open(sanitize_healing_path(file_path), "w", encoding="utf-8") as f:
