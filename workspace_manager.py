...
def get_safe_path(base_dir, user_path):
    ...
    target = os.path.abspath(os.path.join(base, os.path.basename(user_path)))
    return target if target.startswith(base) else os.path.join(base, "default.json")
import os
import json
import shutil
...
class MultiTenantWorkspaceManager:
    ...
    def save_tenant_telemetry(self, client_id, telemetry_data):
        ...
        with open(get_safe_path(".", filepath), "w") as f:
